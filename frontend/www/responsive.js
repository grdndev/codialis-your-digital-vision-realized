...
(function () {
  ...
  function realLangButtons() {
    return [].slice.call(document.querySelectorAll('#cod-nav .cod-navtoggle button'));
  }
  ...
  function build() {
    ...
    function fillPanel() {
      ...
      document.querySelectorAll('#cod-nav .cod-navlinks a').forEach(function (a) {
    ...
    logo.src = existing ? existing.getAttribute('src') : 'assets/codialis-white.png';
    ...
    ['FR', 'EN'].forEach(function (code) {
      ...
      b.addEventListener('click', function () {
        var real = realLangButtons().filter(function (r) { return r.textContent.trim() === code; })[0];
        if (real) real.click();
    ...
    function syncLang() {
      realLangButtons().forEach(function (r) {
        ...
        var active = getComputedStyle(r).backgroundColor.replace(/\s/g, '') === 'rgb(47,237,127)';
        myLang[code].classList.toggle('active', active);
      });
    }

    var burger = document.createElement('button');
    burger.type = 'button'; burger.className = 'cod-mb-burger';
    ...
    burger.addEventListener('click', function (e) {
      ...
      else { fillPanel(); panel.classList.add('open'); }
    });
  ...
  function init() {
    if (build()) return;
    var tries = 0;
    var t = setInterval(function () { if (build() || ++tries > 40) clearInterval(t); }, 150);
  }
